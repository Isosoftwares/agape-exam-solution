...
function Classes({ clientId }) {
  ...
  const getClientOrders = async () => {
    return await axios.get(
      `/classes?page=${activePage}&perPage=${perPage}&status=${status}&clientId=${clientId}`
    );
  };
  ...
  const {
    ...
    queryKey: [`class-orders-${clientId}`],
  ...
  return (
    <div>
      <p className="font-bold  ">Classes Orders </p>
      {/* table */}
      <div className="  ">
        {/* table */}
          ...
          <table className="w-full text-md text-left   ">
            ...
            <tbody className="">
              {loadingClasses || refetchingOrders ? (
                ...
                <tr>
                  <td colSpan={6} className="text-gray-800 text-center py-3  ">
                    ...
                    >
                      ...
                      <td className={`  py-2 px-1 text- text-start`}>
                        <Link
                          ...
                          to={`/dashboard/orders/classes/${item?._id}`}
                        >
                          {item?.classType}
                        </Link>
                      </td>
