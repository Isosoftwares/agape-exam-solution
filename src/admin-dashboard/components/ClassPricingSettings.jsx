...
import { Modal, Button, Group, Loader, Checkbox } from "@mantine/core";
import { useDisclosure } from "@mantine/hooks";
import useAxiosPrivate from "../../hooks/useAxiosPrivate";

function ClassPricingSettings() {
  ...
  const addPricing = (typedata) => {
    return axios.post(`/class-type`, typedata);
  };
  ...
  const {
    ...
    onSuccess: (response) => {
      ...
      queryClient.invalidateQueries(["class-prices-"]);
      addClose();
      reset();
    },
    onError: (err) => {
  ...
  const deletePricing = (price) => {
    return axios.delete(`/class-type/${priceId}`, price);
  };
  ...
  const editPricing = (data) => {
    return axios.patch(`/class-type`, data);
  };
  ...
  const { isLoading: loadingExamPrices, data: pricingData } = useQuery({
    queryFn: getPricings,
    queryKey: [`class-prices-`],
    keepPreviousData: true,
  });

  const submitPricing = (data) => {
    data.paidWeekly = paidWeekly;
    ...
    <div className="bg-light py-3 px-2 ">
              ...
              <div className="flex flex-col w-full ">
                ...
                <div className="flex gap-3 items-center ">
                  <Checkbox
                    checked={paidWeekly}
                    onChange={(event) =>
                      setPaidWeekly(event.currentTarget.checked)
                    }
                  />
                  <label htmlFor="">
                    Paid Weekly (Check if this class type is paid weekly)
                  </label>
                </div>
                <div>
                  {loadingEdit ? (
        ...
        <form action="" onSubmit={handleSubmit(submitPricing)} className="   ">
          <div className="flex flex-col gap-2  ">
            <div className="flex flex-col justify-center gap-2 w-full ">
              ...
              <input
                type="text"
                placeholder="class type name eg. self paced"
                className="border w-full rounded-md py-[5px]  px-2 outline-none  focus:border-blue-700 focus:border-1 "
                {...register("name", {
                  required: true,
                })}
            ...
            <div className="flex gap-3 items-center ">
              <Checkbox
                checked={paidWeekly}
                onChange={(event) => setPaidWeekly(event.currentTarget.checked)}
              />
              <label htmlFor="">
                Paid Weekly (Check if this class type is paid weekly)
              </label>
      ...
      <div className="">
        <div className="flex justify-between items-center  border-b pb-1">
          <p className="font-bold">Class Pricing</p>
          <button
            onClick={() => {
              addOpen();
            }}
        ...
        <div className="">
          <h1 className="capitalize">Class types </h1>
                        ...
                        <td className="border-collapse text-center  gap-3 border-slate-500 py-1 px-3">
                          <span
                            onClick={() => {
                              ...
                              setPaidWeekly(item?.paidWeekly || false);
                              editOpen();
                            }}
...
export default ClassPricingSettings;
