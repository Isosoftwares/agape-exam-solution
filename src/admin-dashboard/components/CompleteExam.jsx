...
function order({ order, closeModal }) {
  ...
  // edit function
  const complete = (data) => {
    return axios.patch("/exams/complete-exam", data);
  };
  ...
  const {
    ...
    onSuccess: (response) => {
      reset();
      queryClient.invalidateQueries([`exam-details-${order?._id}`]);
    ...
    <div className="bg-light py-3">
      <div className="">
        <div className=" min-h- ">
          ...
          >
            <div>
              <p>Mark Exam as Completed</p>
              <p className="font-bold my-3">ExamNo :#{order?.examNo}</p>
            </div>
