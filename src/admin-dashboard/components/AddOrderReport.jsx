...
function AddOrderReport({ link, handleClose, orderId, report }) {
  ...
  const {
    ...
    onSuccess: (response) => {
      queryClient.invalidateQueries([`class-details-${orderId}`]);
      handleClose();
      ...
      <form onSubmit={handleSubmit(onSubmitting)}>
        <div className="flex flex-col w-full">
          ...
          <p className="text-red-500 text-xs mt-1 ">
            {errors.report?.type === "required" && "Report select a writer"}
          </p>
        </div>

        <div className="flex justify-center items-center gap-4 mt-[50px] ">
          <button
            disabled={loadingAddReport || !link}
            type="submit"
