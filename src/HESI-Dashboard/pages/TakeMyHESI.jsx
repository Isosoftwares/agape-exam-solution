...
function TakeMyHESI() {
  ...
  const [testType, setTestType] = useState("HESI");
  ...
  const handleSendMessage = () => {
    ...
    const message = `Hello, I need help with my HESI exam:\n
      - Grade: ${grade}\n
  ...
  const { mutate: testMutate, isPending: loadingTest, error } = useMutation({
    ...
    onSuccess: (response) => {
      if (action === "loggedin") {
        ...
        localStorage.setItem("activeDashboard", "/hesi");
        const text = `Payment link generated!`;
        toast.success(text);
        window.open(url, "_blank");
        setTimeout(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-xl font-bold text-primary">
          Need Help With Your HESI Exam?
        </h2>
        <p className="text-dark/70">Describe your HESI exam requirements</p>
      </div>

      {/* Main Form Container */}
      <div className="bg-white rounded-2xl border border-tertiary/20 shadow-lg overflow-hidden">
        {auth?.userId ? (
          /* Logged In User Form */
          ...
          <div className="p-2">
            ...
            >
              <Stepper.Step
                label="Exam Details"
                description="Tell us about your HESI exam"
              >
                <div className="space-y-6">
                  {/* Form Fields */}
                  <div className="grid md:grid-cols-2 gap-6">
