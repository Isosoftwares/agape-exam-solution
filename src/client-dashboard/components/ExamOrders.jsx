...
import ExamOrderCard from "./order-cards/ExamOrderCard";
...
function ExamOrders() {
  ...
  const getClientOrders = async () => {
    return await axios.get(
      `/exams?page=${activePage}&perPage=${perPage}&clientId=${clientId}&status=${status}`
    );
  };
        ...
        <div className="w-full lg:basis-3/4 ">
          {loadingExams || refetchingOrders ? (
            ...
            <div className="flex flex-col justify-center bg-light min-h-[200px] rounded-md shadow-sm border-2 shadow-cyan-50">
              <p className="text-center ">{examsData?.data?.message}</p>
              <Link
