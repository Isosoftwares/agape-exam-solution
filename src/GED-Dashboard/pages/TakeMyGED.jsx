...
function TakeMyGED() {
  ...
  const [testType, setTestType] = useState("GED");
  ...
  const allSubjects = ["Math", "Science", "Social Studies", "English"];
  const isFormComplete = grade && urgency && subjects.length > 0;
  ...
  function calculateTotalCost(grade, urgency, subjects) {
    let basePrice = 0;

    if (grade === "College Level") {
      if (urgency === "0-3 days") {
        basePrice = 220;
      } else if (urgency === "4-7 days") {
        basePrice = 200;
      } else if (urgency === "Above a week") {
        basePrice = 180;
      }
    } else if (grade === "Pass") {
      if (urgency === "0-3 days") {
    ...
    const totalCost = basePrice * subjects.length;
    return totalCost;
  }
  ...
  const { mutate: testMutate, isPending: loadingTest, error } = useMutation({
    ...
    onSuccess: (response) => {
      if (action === "loggedin") {
        ...
        localStorage.setItem("activeDashboard", "/ged");
        const text = `Payment link generated!`;
        toast.success(text);
        window.open(url, "_blank");
        setTimeout(() => {
  ...
  const signinSubmit = async (data) => {
    ...
    data.amount = calculateTotalCost(grade, urgency, subjects);
    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-xl font-bold text-primary">
          Need Help With Your GED Exam?
        </h2>
        <p className="text-dark/70">Describe your GED exam requirements</p>
      </div>

      {/* Main Form Container */}
      <div className="bg-white rounded-xl border border-tertiary/20 overflow-hidden">
        {auth?.userId ? (
          /* Logged In User Form */
          <div className="">
            {/* Form Fields */}
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Above a week">Above a week</option>
                </select>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="w-full px-4 py-3 bg-secondary/10 border border-secondary/30 rounded-xl">
                    <span className="text-2xl font-bold text-secondary">
                      ${calculateTotalCost(grade, urgency, subjects)}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="p-2">
            ...
            >
              <Stepper.Step
                label="Exam Details"
                description="Tell us about your GED exam"
              >
                <div className="space-y-6">
                  {/* Form Fields - Same as logged in version */}
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-dark mb-2">
                        Grade Level
